  ...
  message.mentions.users.forEach((user) => {
    if (afk.indexOf(user.id) != -1) {
      message.channel.send(
        user.tag +
          " is away right now and may not see your message. Try sending them a DM if it's important to ensure they'll be able to find the message when they return."
      );
    }
  });
    ...
    switch (cmd.toLowerCase()) {
      ...
      case "words":
        switch (args[0]) {
          ...
          default:
            message.channel.send(
              "Hmm... I don't know that command. Try `.help` to get a list of commands."
            );
            break;
        }
        break;
        ...
        if (args[0] && args[0].toLowerCase() != "me") {
          if (parseInt(args[0])) {
            ...
            message.channel.members.forEach((member) => {
              if (
                member.displayName.toLowerCase().indexOf(name.toLowerCase()) !=
                  -1 ||
